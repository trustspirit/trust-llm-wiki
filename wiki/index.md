---
title: Wiki Index
type: index
updated: 2026-05-18
---

# Wiki Index

> 위키의 최상위 허브. 새 페이지가 생기면 해당 섹션에 wikilink를 추가한다.

## People

- [[karpathy]] — [[llm-wiki]] 패턴 제안자
- [[vannevar-bush]] — 1945년 [[memex]] 비전 제안자
- [[gerrit-gong]] — 예수 그리스도 후기성도 교회 사도, [[faith-and-ethics-ai-evaluation]] 제안자
- [[david-bednar]] — 예수 그리스도 후기성도 교회 사도, [[things-as-they-really-are]] 신학 프레임

## Concepts

- [[llm-wiki]] — LLM이 유지하는 persistent markdown 위키 패턴
- [[rag]] — 쿼리 시점에 retrieval하는 표준 방식 ([[llm-wiki]]의 대조항)
- [[memex]] — Bush의 1945년 개인 지식 저장소 비전
- [[agi]] — Artificial General Intelligence / ASI 포함, frontier AI 경쟁의 목표
- [[chatgpt]] — OpenAI의 대화형 generative AI (2022-11 출시)
- [[faith-and-ethics-ai-evaluation]] — 다원주의 신앙·윤리 AI 벤치마크 (7개 루브릭)
- [[church-ai-guiding-principles]] — 예수 그리스도 후기성도 교회의 2024 공식 AI 사용 원칙 (7-원칙/4-영역)
- [[covenant-belonging]] — Gong의 Thou–They–It–I 4원 관계 프레임
- [[moral-agency]] — 자녀가 agent로 행동할 수 있도록 신이 부여한 power of independent action
- [[things-as-they-really-are]] — Bednar의 디지털·AI 시대 진리·실재 분석 프레임
- [[ai-companions]] — AI 가상 동반자의 영적·관계적 위험
- [[deepfakes]] — AI가 만든 위조 인물·미디어, Elder Gong의 3가지 면역 자원
- [[divine-learning-pattern]] — search → ponder → ask → confirmed by Holy Ghost → act
- [[antiqua-et-nova]] — 바티칸 AI 문서, Gong이 인용

## Topics

- [[personal-knowledge-management]] — 개인 지식 관리 일반
- [[ai-ethics]] — AI 윤리·책임 있는 AI 도메인
- [[ai-and-faith]] — AI와 신앙·종교의 교차 영역

---

## 메타

- 첫 페이지가 만들어지기 전까지는 이 파일이 비어있는 게 정상.
- `wiki-ingest`가 새 페이지를 만들면 자동으로 여기에 wikilink를 추가해야 한다 (SCHEMA.md §Ingest 작업 순서 5단계).
